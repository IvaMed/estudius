...
class EstudiusApp {
  ...
  async renderHomePage(main) {
    ...
    // Cargar profesores recomendados
    await this.loadRecommendedTeachers();

    // Event listeners para búsqueda
    document.querySelector('.btn-search-home').addEventListener('click', () => {
  ...
  async loadRecommendedTeachers() {
    try {
      ...
      document.getElementById('recommendedTeachers').innerHTML = 
        '<p>Error al cargar profesores</p>';
    }
  }
  ...
  validateAddTeacherForm() {
    ...
    if (!email || !email.includes('@')) errors.push('Email válido es requerido');
  ...
  showFormWarnings(errors) {
    ...
    // Agregar listeners a los campos para limpiar advertencias cuando se editan
    const form = document.getElementById('addTeacherForm');
    if (form) {
      const inputs = form.querySelectorAll('input, textarea, select');
      inputs.forEach(input => {
        const listener = () => {
          // Re-validar y si no hay errores, ocultar advertencias
          const currentErrors = this.validateAddTeacherForm();
      ...
      // También para checkboxes de materias
    ...
    // Scroll a las advertencias
    warningsDiv.scrollIntoView({ behavior: 'smooth' });
  }
  ...
  async handleAddTeacher(e) {
    ...
    if (!validateAge(formData.age)) {
      setFieldError('age', 'Edad debe estar entre 1 y 149');
      hasErrors = true;
    }
    ...
    if (hasErrors) {
      ...
      const form = document.getElementById('addTeacherForm');
      const firstErrorField = form.querySelector('.error input, .error textarea, .error select');
      if (firstErrorField) {
            ...
            <div class="detail-info">
              ...
              <div class="detail-info-section">
                ...
                <div class="detail-info-item">
                  ...
                  <span class="detail-info-value">${teacher.classSize}</span>
                </div>
                <div class="detail-info-item">
