...
function canonicalSchedule(obj) {
  const slots = Array.isArray(obj.slots)
    ? obj.slots
        ...
        .filter((s) => s.start && s.end && s.dow >= 0 && s.dow <= 6 && timeToMinutes(s.end) > timeToMinutes(s.start))
    : [];
  return {
    version: 1,
    slots,
...
function migrateLegacyScheduleText(text) {
  ...
  if (timeRange) {
    ...
    if (a && b && timeToMinutes(b) > timeToMinutes(a)) {
      startH = a;
      endH = b;
    }
  }
...
function teacherMatchesAvailability(scheduleObj, targetDows, filterStartMin, filterEndMin) {
  ...
  for (const slot of sch.slots) {
    ...
    if (rangesOverlapMinutes(s, e, filterStartMin, filterEndMin)) return true;
  }
  return false;
}
 ...
 * Si hay más de una franja el mismo día de la semana, se usa solo la primera (producto no contempla dos el mismo día).
 */
